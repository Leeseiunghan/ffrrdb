...
def getToHours(hours):
    fromHours = int(hours[:2])
    backStr = hours[2:]
    ...
    return lpad(str(toHours), 2, '0')+backStr
...
async def my_background_task():
	...
	while True:                
		...
		#await message.channel.send(222)
			...
			for i in range(bossNum):
				...
				if bossTime[i] <= priv0 and bossTime[i] > priv:
					if basicSetting[3] != '0':
						if bossFlag0[i] == False:
							bossFlag0[i] = True
							await message.channel.send(bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + bossData[i][3], tts=False)
				...
				if bossTime[i] <= priv and bossTime[i] > now:
					if basicSetting[1] != '0' :
						if bossFlag[i] == False:
							bossFlag[i] = True
							await message.channel.send(bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + bossData[i][3], tts=False)
				...
				if bossMungFlag[i] == True:
					if (bossTime[i]+datetime.timedelta(days=-365)) <= aftr:
						if bossData[i][2] == '0':
							await message.channel.send(bossData[i][0] + ' 미입력 됐습니다.', tts=False)
...
async def on_message(msg):
	...
	if chflg == 0 :
		...
		await msg.channel.send('< ' + client.get_channel(channel).name + ' 접속완료>', tts=False)                
	...
	if client.get_channel(channel) != msg.channel :
		...
		##################################						
			if message.content.startswith(bossData[i][0] +'삭제'):
				...
				await message.channel.send('<' + bossData[i][0] + ' 삭제완료>', tts=False)
				await dbSave()
		...
		if message.content.startswith('!명치'):
			...
			await message.channel.send('<재접속 성공>', tts=False)
			...
			if LoadChk == 0:
				await message.channel.send('<불러오기 완료>', tts=False)
			else:
				await message.channel.send('<보스타임 정보가 없습니다.>', tts=False)
		...
		if message.content.startswith('!초기화'):
			...
			await message.channel.send('<초기화 완료>', tts=False)
			print ("<초기화 완료>")
		...
		if message.content.startswith('!보스일괄'):
			...
			await message.channel.send('<보스 일괄 입력 완료>', tts=False)
			print ("<보스 일괄 입력 완료>")
		...
		if message.content.startswith('!ㅂ') or message.content.startswith('!q') or message.content.startswith('!ㅃ') or message.content.startswith('!Q'):
			await dbLoad()
				...
				for i in range(bossNum):
					if timestring == bossTime[i]:
						if bossTimeString[i] != '99:99:99' :
							if bossData[i][2] == '0' :
								if bossMungCnt[i] == 0 :
									information += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' - ' + getToHours(bossTimeString[i]) + '\n'
								else :
									information += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' - ' + getToHours(bossTimeString[i]) + ' (미입력 ' + str(bossMungCnt[i]) + '회)' + '\n'
							else : 
								if bossMungCnt[i] == 0 :
									information += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' - ' + getToHours(bossTimeString[i]) + '\n'
								else :
									information += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' - ' + getToHours(bossTimeString[i]) + ' (멍 ' + str(bossMungCnt[i]) + '회)' + '\n'
		...
		if message.content.startswith('!현재시간'):
			await message.channel.send(datetime.datetime.now().strftime('%H:%M:%S'), tts=False)
